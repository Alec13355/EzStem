...
public class ProductionSheetTests
{
    ...
    public async Task GetProductionSheet_OneRecipeOneItem_ReturnsCorrectQuantities()
    {
        ...
        var recipe = new Recipe { Id = Guid.NewGuid(), Name = "Bridal Bouquet" };
        ...
        var evt = new FloristEvent { Id = Guid.NewGuid(), Name = "Spring Wedding", EventDate = DateTime.UtcNow.AddDays(30), ClientName = "Alice" };
        ...
        var result = await service.GetProductionSheetAsync(evt.Id);
    ...
    public async Task GetProductionSheet_TwoRecipes_QuantitiesMultiplyCorrectly()
    {
        ...
        var recipe1 = new Recipe { Id = Guid.NewGuid(), Name = "Bouquet A" };
        var recipe2 = new Recipe { Id = Guid.NewGuid(), Name = "Centerpiece B" };
        ...
        var evt = new FloristEvent { Id = Guid.NewGuid(), Name = "Gala", EventDate = DateTime.UtcNow.AddDays(60) };
        context.Events.Add(evt);

        // 3 of Bouquet A, 4 of Centerpiece B
    ...
    public async Task GetProductionSheet_NonExistentEvent_ReturnsNull()
    {
        ...
        var result = await service.GetProductionSheetAsync(Guid.NewGuid());
    ...
    public async Task GetProductionSheet_MultipleQuantitiesSameRecipe_StacksCorrectly()
    {
        ...
        var recipe = new Recipe { Id = Guid.NewGuid(), Name = "Table Arrangement", Description = "Use fresh peonies" };
        context.Recipes.Add(recipe);

        // Recipe needs 8 stems per arrangement
        var recipeItem = new RecipeItem { Id = Guid.NewGuid(), RecipeId = recipe.Id, ItemId = item.Id, Quantity = 8, CostPerStem = 1.2m };
        context.RecipeItems.Add(recipeItem);

        var evt = new FloristEvent { Id = Guid.NewGuid(), Name = "Anniversary Party", EventDate = DateTime.UtcNow.AddDays(14) };
        context.Events.Add(evt);
