...
public class OrderService : IOrderService
{
    ...
    public async Task<OrderResponse> GenerateOrderAsync(Guid eventId, string ownerId, CancellationToken ct = default)
    {
        ...
        var flexItems = await _context.FlexItems
            .Include(f => f.Item).ThenInclude(i => i.Vendor)
        ...
        return await GetOrderAsync(order.Id, ownerId, ct) ?? throw new Exception("Failed to retrieve created order");
    }

    public async Task<OrderResponse?> GetOrderAsync(Guid id, string ownerId, CancellationToken ct = default)
    {
        var order = await _context.Orders
            .Include(o => o.Event)
            .Include(o => o.LineItems).ThenInclude(li => li.Item)
            .Include(o => o.LineItems).ThenInclude(li => li.Vendor)
            .FirstOrDefaultAsync(o => o.Id == id && o.OwnerId == ownerId, ct);

        if (order == null) return null;
        return MapToOrderResponse(order);
    }

    public async Task<PagedResponse<OrderResponse>> GetOrdersAsync(int page, int pageSize, string ownerId, Guid? eventId = null, CancellationToken ct = default)
    {
        ...
        return new PagedResponse<OrderResponse>(orders.Select(MapToOrderResponse), total, page, pageSize);
    }

    private OrderResponse MapToOrderResponse(Order order)
    {
        var lineItems = order.LineItems.Select(li => new OrderLineItemResponse(
            li.Id, li.ItemId, li.Item.Name, li.VendorId, li.Vendor?.Name,
            li.QuantityNeeded, li.QuantityOrdered, li.Item.BundleSize,
            (int)Math.Ceiling(li.QuantityNeeded / li.Item.BundleSize),
            li.CostPerUnit, li.QuantityOrdered * li.CostPerUnit)).ToList();

        var byVendor = lineItems
            .GroupBy(li => new { li.VendorId, li.VendorName })
            .Select(g => new VendorOrderGroup(g.Key.VendorId, g.Key.VendorName ?? "No Vendor", g.ToList(), g.Sum(li => li.LineTotalCost)))
            .ToList();

        var totalCost = lineItems.Sum(li => li.LineTotalCost);

        return new OrderResponse(
            order.Id, order.EventId, order.Event.Name, order.Status.ToString(),
            totalCost, lineItems, byVendor,
            order.WastePercentage, order.WasteCalculationDate, order.CreatedAt);
    }

    public async Task<WasteSummary> CalculateWasteAsync(Guid orderId, string ownerId, decimal actualStemsUsed, CancellationToken ct = default)
    {
        var order = await _context.Orders
            .Include(o => o.LineItems)
            .FirstOrDefaultAsync(o => o.Id == orderId && o.OwnerId == ownerId, ct);
    ...
    public async Task<WasteSummary?> GetWasteAsync(Guid orderId, string ownerId, CancellationToken ct = default)
    {
        var order = await _context.Orders
            .Include(o => o.LineItems)
            .FirstOrDefaultAsync(o => o.Id == orderId && o.OwnerId == ownerId, ct);
    ...
    private static (IEnumerable<string> suggestions, decimal multiplier) GetOptimizationSuggestions(decimal wastePercentage)
    {
        ...
        if (wastePercentage > 30)
            return (new[] { $"You had {pct}% waste — consider ordering 20% fewer stems next time.", "Review your recipe quantities — they may be set too high." }, 0.75m);

        if (wastePercentage > 20)
            return (new[] { $"You had {pct}% waste — try ordering 15% fewer stems next time." }, 0.82m);

        if (wastePercentage >= 10)
            return (new[] { $"Minor waste ({pct}%). Consider reducing orders by 10% for leaner buying." }, 0.90m);

        if (wastePercentage >= 5)
            return (new[] { "Good efficiency! A small 5% buffer reduction could save costs." }, 0.95m);

        return (new[] { "Excellent efficiency — you're using nearly everything you order." }, 1.0m);
    }

    public async Task<string> GenerateOrderCsvAsync(Guid orderId, string ownerId, CancellationToken ct = default)
    {
        var order = await _context.Orders
            .Include(o => o.Event)
            .Include(o => o.LineItems).ThenInclude(li => li.Item)
        ...
        var lineItems = order.LineItems.OrderBy(li => li.Vendor?.Name ?? "No Vendor").ThenBy(li => li.Item.Name).ToList();
        decimal grandTotal = 0;

        foreach (var lineItem in lineItems)
        {
            var vendorName = lineItem.Vendor?.Name ?? "No Vendor";
            var bundlesOrdered = (int)Math.Ceiling(lineItem.QuantityNeeded / lineItem.Item.BundleSize);
            var totalCost = lineItem.QuantityOrdered * lineItem.CostPerUnit;
            grandTotal += totalCost;

            csv.AppendLine($"\"{vendorName}\",\"{lineItem.Item.Name}\",{lineItem.Item.BundleSize},{bundlesOrdered},{lineItem.QuantityOrdered},${lineItem.CostPerUnit:F2},${totalCost:F2}");
        }
